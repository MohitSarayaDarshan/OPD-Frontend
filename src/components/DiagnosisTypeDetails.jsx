  ...
  return (
    <div className={`min-h-screen bg-gray-50 text-slate-800 font-sans p-8 ${expanded ? "ml-64" : "ml-16"} transition-all duration-1000 animate-fade-in`}>
      ...
      <div className="flex items-center gap-4 mb-8 animate-slide-down">
        <button 
          onClick={() => navigate('/admin/getAllDiagnosisTypes')}
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Diagnosis Type Details</h1>
          <p className="text-sm text-slate-500 mt-1">View information for Diagnosis Type ID: #{diagnosisData.DiagnosisTypeID}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6 animate-scale-in">
          ...
          <div className="bg-white rounded-xl border border-blue-100 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden">
            <div className="h-32 bg-gradient-to-r from-red-600 to-red-500 relative"></div>
            <div className="px-8 pb-8">
              <div className="relative -mt-12 mb-4">
                <div className="w-24 h-24 bg-white rounded-full p-1.5 shadow-md">
                   <div className="w-full h-full bg-red-50 rounded-full flex items-center justify-center text-red-600">
              ...
              <div className="flex justify-between items-start">
                <div>
                  <h2 className="text-2xl font-bold text-slate-900">{diagnosisData.DiagnosisTypeName}</h2>
                  <div className="flex items-center gap-2 text-slate-500 mt-1">
                    <Hash className="w-4 h-4" />
                    <span className="font-mono">{diagnosisData.DiagnosisTypeShortName}</span>
                  </div>
                </div>
                <span className={`px-3 py-1 text-xs font-bold uppercase tracking-wide rounded-full border ${
                  ...
                  {diagnosisData.IsActive ? 'Active' : 'Inactive'}
                </span>
              </div>

              <div className="mt-6 pt-6 border-t border-gray-100">
                <h4 className="text-sm font-bold text-slate-900 flex items-center gap-2 mb-2">
                  <FileText className="w-4 h-4 text-red-500" /> Description
                </h4>
                <p className="text-sm text-slate-600 leading-relaxed">
                  {diagnosisData.Description || "No description provided."}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-blue-100 shadow-sm hover:shadow-md transition-all duration-300 p-6">
            ...
            <div className="grid grid-cols-1 gap-4">
               <div className="p-4 bg-gray-50 rounded-lg border border-gray-100">
                 ...
                 <p className="text-lg font-mono font-bold text-slate-700 mt-1">#{diagnosisData.HospitalID}</p>
                 <p className="text-sm text-slate-500 mt-1">{diagnosisData.HospitalName}</p>
               </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-1 space-y-6 animate-scale-in">
          ...
          <div className="bg-white rounded-xl border border-blue-100 shadow-sm hover:shadow-md transition-all duration-300 p-6">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center py-2 border-b border-gray-50">
                <span className="text-xs text-slate-500">Diagnosis Type ID</span>
                <span className="text-sm font-mono font-medium text-slate-700">#{diagnosisData.DiagnosisTypeID}</span>
              </div>

              <div className="flex justify-between items-center py-2 border-b border-gray-50">
                <span className="text-xs text-slate-500">Status</span>
                <span className={`flex items-center gap-1 text-xs font-medium ${
                  diagnosisData.IsActive ? 'text-green-600' : 'text-gray-600'
                }`}>
              ...
              <div className="flex justify-between items-center py-2 border-b border-gray-50">
                ...
                <span className="text-sm font-mono font-medium text-slate-700">ID: {diagnosisData.UserID}</span>
              </div>
              ...
              <div className="py-2">
                ...
                <div className="flex items-center gap-2 text-sm text-slate-700">
                   ...
                   {formatDate(diagnosisData.Created)}
                </div>
              </div>
              ...
              <div className="py-2">
                ...
                <div className="flex items-center gap-2 text-sm text-slate-700">
                   ...
                   {formatDate(diagnosisData.Modified)}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-red-50 rounded-xl border border-red-100 p-6">
             <h4 className="text-sm font-bold text-red-900 mb-3">Actions</h4>
             <div className="space-y-2">
                <button 
                  onClick={() => navigate(`/admin/editDiagnosisType/${diagnosisData.DiagnosisTypeID}`)}
                  className="w-full flex items-center justify-center gap-2 py-2 bg-white text-red-600 text-sm font-medium rounded-lg border border-red-200 hover:bg-red-600 hover:text-white transition-all duration-300 shadow-sm transform hover:scale-105 active:scale-95"
                >
...
export default DiagnosisTypeDetails;