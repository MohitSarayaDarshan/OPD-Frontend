...
const DoctorDetails = () => {
  ...
  useEffect(() => {
  // 1. Safe Guard: Ensure doctorData exists before doing anything
  if (!doctorData) return;

  // 2. Prepare the Staff Promises (only if StaffID array exists)
  const staffPromises = doctorData.StaffID && doctorData.StaffID.length > 0
    ? doctorData.StaffID.map((element) =>
        fetch("http://localhost:3000/api/staffs/" + element, { credentials: 'include' })
          .then((res) => {
            if (!res.ok) throw new Error(`Staff fetch failed: ${res.status}`);
            return res.json();
          })
          .then((json) => json?.StaffName || "Unknown Staff")
      )
    : []; // Fallback to an empty array if no staff IDs

  // 3. Prepare the Hospital Promise (only if HospitalID exists)
  const hospitalPromise = doctorData.HospitalID
    ? fetch("http://localhost:3000/api/hospitals/" + doctorData.HospitalID, { credentials: 'include' })
        .then((res) => {
          if (!res.ok) throw new Error(`Hospital fetch failed: ${res.status}`);
          return res.json();
        })
        .then((json) => {setHospitalData(json)})
    : Promise.resolve(null); // Resolves instantly with null if no hospital ID

  // 4. Run everything in parallel safely
  Promise.all([Promise.all(staffPromises), hospitalPromise])
    .then(([namesArray, hospitalName]) => {
      // Both datasets arrived successfully! Update state together.
      setStaffData(namesArray);
      
      if (hospitalName) {
        setHospitalData(hospitalName);
      }
    })
    .catch((err) => {
      console.error("Error updating doctor-related data:", err);
    });
          ...
          <div className="bg-white rounded-xl border border-blue-100 shadow-sm hover:shadow-md transition-all duration-300 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
               <div className="p-4 bg-gray-50 rounded-lg border border-gray-100">
                 <p className="text-xs text-slate-400 font-medium uppercase">Staffs</p>
                 <p className="text-lg font-mono font-bold text-slate-700 mt-1">{staffData && staffData.length > 0 ? staffData.join(', ') : "N/A"}</p>
               </div>
               <div className="p-4 bg-gray-50 rounded-lg border border-gray-100">
               ...
               <div className="p-4 bg-gray-50 rounded-lg border border-gray-100">
                 <p className="text-xs text-slate-400 font-medium uppercase">Hospital</p>
                 <p className="text-lg font-mono font-bold text-slate-700 mt-1">{hospitalData.HospitalName}</p>
               </div>
            </div>
          </div>
