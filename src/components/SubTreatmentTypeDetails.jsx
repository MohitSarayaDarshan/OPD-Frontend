  ...
  return (
    <div className={`min-h-screen bg-gray-50 text-slate-800 font-sans p-8 ${expanded ? "ml-64" : "ml-16"} transition-all duration-1000 animate-fade-in`}>
      ...
      <div className="flex items-center gap-4 mb-8 animate-slide-down">
        <button 
          onClick={() => navigate('/admin/getAllSubTreatments')}
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Sub-Treatment Type Details</h1>
          <p className="text-sm text-slate-500 mt-1">View information for Sub-Treatment Type ID: #{subTreatmentData.SubTreatmentTypeID}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6 animate-scale-in">
          ...
          <div className="bg-white rounded-xl border border-blue-100 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden">
            <div className="h-32 bg-gradient-to-r from-orange-600 to-orange-500 relative"></div>
            <div className="px-8 pb-8">
              <div className="relative -mt-12 mb-4">
                <div className="w-24 h-24 bg-white rounded-full p-1.5 shadow-md">
                   <div className="w-full h-full bg-orange-50 rounded-full flex items-center justify-center text-orange-600">
              ...
              <div className="flex justify-between items-start">
                <div>
                  <h2 className="text-2xl font-bold text-slate-900">{subTreatmentData.SubTreatmentTypeName}</h2>
                  <div className="flex items-center gap-2 text-slate-500 mt-1">
                    <LinkIcon className="w-4 h-4" />
                    <span>Parent: {subTreatmentData.TreatmentTypeName}</span>
                  </div>
                </div>
              ...
              <div className="mt-6 pt-6 border-t border-gray-100">
                ...
                <p className="text-sm text-slate-600 leading-relaxed">
                  {subTreatmentData.Description || "No description provided."}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-blue-100 shadow-sm hover:shadow-md transition-all duration-300 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               <div className="p-4 bg-gray-50 rounded-lg border border-gray-100">
                 <p className="text-xs text-slate-400 font-medium uppercase">Parent Treatment Type ID</p>
                 <p className="text-lg font-mono font-bold text-slate-700 mt-1">#{subTreatmentData.TreatmentTypeID}</p>
                 <p className="text-sm text-slate-500 mt-1">{subTreatmentData.TreatmentTypeName}</p>
               </div>
               <div className="p-4 bg-gray-50 rounded-lg border border-gray-100">
                 <p className="text-xs text-slate-400 font-medium uppercase">Linked Account ID</p>
                 <p className="text-lg font-mono font-bold text-slate-700 mt-1">#{subTreatmentData.AccountID}</p>
               </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-1 space-y-6 animate-scale-in">
          ...
          <div className="bg-white rounded-xl border border-blue-100 shadow-sm hover:shadow-md transition-all duration-300 p-6">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center py-2 border-b border-gray-50">
                <span className="text-xs text-slate-500">Sub-Treatment Type ID</span>
                <span className="text-sm font-mono font-medium text-slate-700">#{subTreatmentData.SubTreatmentTypeID}</span>
              </div>
              ...
              <div className="flex justify-between items-center py-2 border-b border-gray-50">
                ...
                <span className="text-sm font-mono font-medium text-slate-700">ID: {subTreatmentData.UserID}</span>
              </div>
              ...
              <div className="py-2">
                ...
                <div className="flex items-center gap-2 text-sm text-slate-700">
                   ...
                   {formatDate(subTreatmentData.Created)}
                </div>
              </div>
              ...
              <div className="py-2">
                ...
                <div className="flex items-center gap-2 text-sm text-slate-700">
                   ...
                   {formatDate(subTreatmentData.Modified)}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-orange-50 rounded-xl border border-orange-100 p-6">
             <h4 className="text-sm font-bold text-orange-900 mb-3">Actions</h4>
             <div className="space-y-2">
                <button 
                  onClick={() => navigate(`/admin/editSubTreatment/${subTreatmentData.SubTreatmentTypeID}`)}
                  className="w-full flex items-center justify-center gap-2 py-2 bg-white text-orange-600 text-sm font-medium rounded-lg border border-orange-200 hover:bg-orange-600 hover:text-white transition-all duration-300 shadow-sm transform hover:scale-105 active:scale-95"
                >
...
export default SubTreatmentTypeDetails;