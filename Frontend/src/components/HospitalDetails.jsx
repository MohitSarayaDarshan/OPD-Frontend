...
function HospitalDetails() {
  ...
  useEffect(
    ()=>{
      ...
      .then((res)=>res.json())
      .then((json)=>{setHospitalData(json[0]);console.log(json)})
    },
    []
  )
  ...
  function handleDelete(id){
    ...
    fetch(`http://localhost:3000/api/hospitals/delete/${id}`,{method:"DELETE"})
    .then(res=>res.json())
    .then((json)=>{alert("Hospital deleted with id="+id);navigate('/admin/getAllHospitals')})
  }
    ...
    <div className={`min-h-screen bg-gray-50 text-slate-800 font-sans p-8 ${expanded ? "ml-64" : "ml-16"} transition-all duration-1000`}>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        ...
        <div className="flex gap-3">
          ...
          onClick={() => navigate(`/admin/editHospital/${id}`)}>
